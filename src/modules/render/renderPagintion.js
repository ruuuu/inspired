...
export const renderPagination = (wrapperPagination, page, pages, count) => {              //    data = { goods: [{},{},{}],  pages: ,  page: ,  totalCount: }
    ...
    const isNotStart = (page - Math.floor(count / 2)) > 1;        //  Math.floor округляет, если мы  не вначале страницы
    // 5 6 7 8
    const isEnd = (page + Math.floor(count / 2)) > pages;         // если мы не в конце спсик стрнаиц
    ...
    for (let i = 0; i < count; i++) {
        ...
        createElement('li',
            ...
            },
            {
                ...
                append: createElement('a', { textContent: n, href: `${router.getCurrentLocation().url}?page=${n}`, className: `pagination__link ${page === n ? 'pagination__link--active' : ''}` })
            }
        );
    }
